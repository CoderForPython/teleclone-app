...
const ChatWindow: React.FC<ChatWindowProps> = ({ targetUser, currentUser, onBack, onStartCall, onOpenAdmin }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white relative w-full overflow-hidden">
      ...
      <div className="p-3 md:p-4 bg-white border-b border-slate-200 flex items-center space-x-2 md:space-x-4 shadow-sm z-20">
        ...
        <img src={targetUser.avatar} className="w-9 h-9 md:w-10 md:h-10 rounded-full object-cover border border-slate-100 flex-shrink-0" alt={targetUser.username} />
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
          {selectionMode && (
            <div className="flex items-center space-x-1 md:space-x-2 mr-1">
              <button 
                ...
                className="px-2.5 py-1.5 md:px-3 rounded-lg text-slate-500 bg-slate-100 hover:bg-slate-200 transition-colors text-xs md:text-sm font-semibold"
              >
                Отмена
              </button>
              <button 
                ...
                className={`px-2.5 py-1.5 md:px-3 rounded-lg text-white font-semibold text-xs md:text-sm transition-all ${
                  selectedIds.size > 0 
                  ? 'bg-red-500 hover:bg-red-600 active:scale-95 shadow-md shadow-red-100' 
          ...
          <div className="relative" ref={menuRef}>
            ...
            {showMenu && (
              <div className="absolute right-0 mt-2 w-52 bg-white border border-slate-100 shadow-2xl rounded-xl py-2 z-50 animate-in fade-in zoom-in duration-100">
                <button 
                  onClick={startSelectionMode}
                  className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 flex items-center space-x-3 transition-colors"
                >
                  <i className="fa-solid fa-trash-can text-slate-400"></i>
                  <span className="font-medium">Удалить сообщения</span>
                </button>
                
                {/* Admin Button only for bot support chat if unlocked */}
                {targetUser.id === 'bot_support' && isAdminUnlocked && (
                  <button 
                    onClick={() => { setShowMenu(false); onOpenAdmin?.(); }}
                    className="w-full text-left px-4 py-2.5 text-sm text-blue-600 hover:bg-blue-50 flex items-center space-x-3 transition-colors border-t border-slate-50"
                  >
        ...
        <div className="relative z-10 flex flex-col space-y-3">
          {messages.map((msg) => {
            ...
            return (
              <div 
                key={msg.id} 
                className={`flex items-center w-full ${isOwn ? 'justify-end' : 'justify-start'} group relative`}
              ...
              >
                {selectionMode && isOwn && (
                  <div className="mr-3 flex-shrink-0 cursor-pointer animate-in fade-in slide-in-from-right-2 duration-200">
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                      isSelected ? 'bg-green-500 border-green-500 text-white' : 'border-slate-300 bg-white shadow-inner'
                    }`}>
                ...
                <div className={`max-w-[85%] md:max-w-[75%] rounded-2xl relative shadow-sm overflow-hidden flex flex-col transition-transform ${
                  isOwn ? 'bg-blue-500 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none border border-slate-100'
                } ${isImage ? 'p-1' : 'px-3 py-1.5'} ${selectionMode ? 'cursor-pointer active:scale-[0.98]' : ''}`}>
                  {isImage && msg.file && (
                  ...
                  {!isImage && msg.file && (
                    <div className="flex items-center space-x-3 my-1">
                      ...
                      {!selectionMode && (
                        <button onClick={(e) => { e.stopPropagation(); downloadFile(msg.file!.data, msg.file!.name); }} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isOwn ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-600'}`}><i className="fa-solid fa-arrow-down text-sm"></i></button>
                      )}
                    </div>
                  )}
      ...
      <div className={`p-3 md:p-4 bg-white border-t border-slate-200 z-10 transition-all ${selectionMode ? 'opacity-40 pointer-events-none grayscale' : ''}`}>
        <div className="flex items-center space-x-2 md:space-x-3">
          ...
          <div className="flex-grow relative">
            <input 
              ...
              className="w-full bg-slate-100 border border-transparent rounded-2xl py-2.5 md:py-3 px-4 focus:bg-white focus:border-blue-200 focus:ring-4 focus:ring-blue-50 outline-none transition-all pr-10 text-sm text-slate-900 font-medium placeholder:text-slate-400" 
            />
          ...
          <button 
            ...
            className={`w-11 h-11 md:w-12 md:h-12 rounded-full flex-shrink-0 flex items-center justify-center transition-all ${
              inputText.trim() 
