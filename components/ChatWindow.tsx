...
const ChatWindow: React.FC<ChatWindowProps> = ({ targetUser, currentUser, onMessageSent, onBack }) => {
  ...
  const handleSendMessage = (text: string, file?: Message['file']) => {
    ...
    db.saveMessage(convId, newMessage);
  ...
  return (
    <div className="flex flex-col h-full bg-white relative w-full overflow-hidden">
      ...
      <div className="p-3 md:p-4 bg-white border-b border-slate-200 flex items-center space-x-2 md:space-x-4 shadow-sm z-20">
        ...
        <button 
          onClick={onBack}
          className="md:hidden text-slate-500 hover:text-blue-500 p-2 -ml-2 transition-colors"
        >
        ...
        <img 
          ...
          className="w-9 h-9 md:w-10 md:h-10 rounded-full object-cover border border-slate-100" 
          alt={targetUser.username}
        />
        <div className="flex-grow min-w-0">
          <h2 className="font-bold text-slate-800 leading-tight truncate text-sm md:text-base">
            {targetUser.username}
          </h2>
        ...
        <div className="flex items-center space-x-3 md:space-x-5 text-slate-400">
          <button className="hidden sm:block hover:text-blue-500 transition-colors"><i className="fa-solid fa-search text-lg"></i></button>
        ...
        <div className="relative z-10 flex flex-col space-y-3">
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                <div className={`max-w-[85%] md:max-w-[75%] rounded-2xl relative shadow-sm group overflow-hidden ${
                  isOwn 
                  ...
                  {isImage && msg.file && (
                    <div className="relative overflow-hidden rounded-xl">
                      <img 
                        src={msg.file.data} 
                        className="max-w-full h-auto max-h-[60vh] md:max-h-96 rounded-xl object-contain block" 
                        alt={msg.file.name} 
                      />
                      {/* Download Button - always visible or hover */}
                      <button 
                        onClick={() => downloadFile(msg.file!.data, msg.file!.name)}
                        className="absolute top-2 right-2 bg-black/40 text-white w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-md opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300 transform"
                      >
                  ...
                  <div className={`text-[9px] mt-0.5 flex items-center justify-end space-x-1 ${
                    isImage ? 'absolute bottom-2 right-3 bg-black/30 px-1.5 py-0.5 rounded-full text-white' : (isOwn ? 'text-blue-100' : 'text-slate-400')
                  }`}>
      ...
      {/* Emoji Picker Popup - Repositioned for mobile */}
      {showEmojiPicker && (
          ...
          <div className="grid grid-cols-5 gap-2">
            {COMMON_EMOJIS.map(emoji => (
              <button 
                ...
                className="text-2xl hover:bg-slate-50 p-2 rounded-xl transition-all active:scale-90"
              >
                {emoji}
              </button>
            ))}
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(inputText); }} className="flex items-center space-x-2 md:space-x-3">
          ...
          <div className="flex-grow relative">
            ...
            <button 
              ...
              className={`absolute right-2 top-1/2 -translate-y-1/2 transition-colors ${showEmojiPicker ? 'text-blue-500' : 'text-slate-400 hover:text-blue-500'}`}
            >
          ...
          <button 
            ...
            className={`w-11 h-11 md:w-12 md:h-12 rounded-full flex-shrink-0 flex items-center justify-center transition-all ${
              inputText.trim() ? 'bg-blue-500 text-white shadow-lg shadow-blue-200 active:scale-95' : 'bg-slate-100 text-slate-300'
            }`}
          >
